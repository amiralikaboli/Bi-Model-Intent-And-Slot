...
for epoch in range(epoch_num):
    slot_loss_history = []
    intent_loss_history = []
    for batch_index, data in tqdm(enumerate(utils.get_batch(train_data))):

        # Preparing data
        sentence, real_len, slot_label, intent_label = data

        mask = utils.make_mask(real_len, len(slot_dict)).to(device)
    ...
    # Evaluation 
    # total_test = len(test_data) // cfg.batch * cfg.batch
    correct_num = 0
    TP, FP, FN = 0, 0, 0
    ...
    for batch_index, data_test in tqdm(enumerate(utils.get_batch(test_data))):
        ...
        mask_test = utils.make_mask(real_len_test, len(slot_dict)).to(device)
        ...
        for spt, slt, rl in zip(slot_pred_test, slot_label_test, real_len_test):
            ...
            pred_chunks = get_chunks(['O'] + spt + ['O'])
            label_chunks = get_chunks(['O'] + slt + ['O'])
